...
import useLocationHandler from '../hooks/Location';
import LocationPrompt from './Location';
import { MapPin } from 'lucide-react'; // ⬅️ Location icon import
...
export default function DiaryOverview() {
  ...
  return (
    <>
      {showPopup && (
        <LocationPrompt onLocationFetched={handleLocationFetched} onClose={() => setShowPopup(false)} />
      )}
      ...
      <div className="relative p-4 sm:p-8 min-h-screen bg-gradient-to-br from-pink-100 via-purple-100 to-indigo-100 overflow-hidden">
            ...
            {entries.map((entry, index) => (
              ...
              >
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <span className="text-sm text-indigo-500">{new Date(entry.date).toLocaleDateString()}</span>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
        ...
        <div className="fixed bottom-8 right-8 flex flex-col gap-4 z-20">
          ...
          <div
            onClick={() => {
              if (!locationEnabled) toggleLocationPopup();
            }}
            className={`p-3 rounded-full shadow-md cursor-pointer transition-all ${
              locationEnabled ? 'bg-green-100 border-green-300 text-green-500' : 'bg-gray-100 border-gray-300 text-gray-500'
            }`}
        ...
        {/* Add Entry Modal */}
        {showAddModal && (
          <AddDiaryEntry onEntryAdded={handleEntryAdded} onClose={() => setShowAddModal(false)} />
        )}
        ...
        {showListModal && (
          <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center px-4">
            <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 relative shadow-2xl animate-fadeIn scale-95">
              ...
              <h2 className="text-2xl font-bold text-indigo-700 mb-4 text-center font-quicksand">
                All Diary Entries
              </h2>
